...
import type { RenderedEntry, HistoryEntry } from "../src/core/render-entries";

const userEntry = (text: string): HistoryEntry => ({
  info: { id: "u", role: "user" },
  parts: [{ type: "text", text }],
});

const assistantEntry = (text: string): HistoryEntry => ({
...
const messages: HistoryEntry[] = [
  userEntry("Fix login bug"),
  assistantEntry("Reading auth.ts"),
  assistantEntry("[Read] code here"),
  assistantEntry("Found the root cause in auth module"),
];
...
describe("searchEntries", () => {
  ...
  it("falls back to escaped literal for invalid regex", () => {
    ...
    const extraMsgs: HistoryEntry[] = [
      userEntry("error with (foo pattern"),
      assistantEntry("no match here"),
    ];
  ...
  it("snippet shows context lines around match", () => {
    ...
    const m: HistoryEntry[] = [userEntry(multiline)];
